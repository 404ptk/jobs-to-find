...
<div id="apply-modal-{{ $jobOffer->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    <div class="fixed inset-0 bg-opacity-30 backdrop-blur-sm transition-opacity" onclick="hideApplyModal({{ $jobOffer->id }})"></div>
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full transform transition-all scale-95 opacity-0 relative" id="apply-modal-content-{{ $jobOffer->id }}">
        <button onclick="hideApplyModal({{ $jobOffer->id }})" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 z-20 cursor-pointer">
        ...
        <div class="p-8">
            ...
            <form id="apply-form-{{ $jobOffer->id }}" action="{{ route('job-offers.apply', $jobOffer->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6" onsubmit="handleApplySubmit(event, {{ $jobOffer->id }})">
                @csrf

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="first_name_{{ $jobOffer->id }}" class="block text-sm font-medium text-gray-700 mb-2">
                            First Name <span class="text-red-500">*</span>
                        </label>
                        <input 
                            type="text" 
                            id="first_name_{{ $jobOffer->id }}" 
                            name="first_name" 
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-blue-400 transition">
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600">
                                <label for="cv_{{ $jobOffer->id }}" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500">
                                    <span>Upload a file</span>
                                    <input id="cv_{{ $jobOffer->id }}" name="cv" type="file" class="sr-only" accept=".pdf,.doc,.docx">
                                </label>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-blue-700">
                                Your application will be sent to the employer. Make sure all information is correct before submitting.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex gap-3 pt-4">
                    <button 
                        type="button" 
                        onclick="hideApplyModal({{ $jobOffer->id }})"
                    ...
                    >
                        ...
                        <svg id="submit-spinner-{{ $jobOffer->id }}" class="hidden animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <div id="success-message-{{ $jobOffer->id }}" class="hidden bg-green-50 border-l-4 border-green-500 p-4 rounded-r-lg">
                ...
                <div id="error-message-{{ $jobOffer->id }}" class="hidden bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg">
                    <div class="flex items-center">
                        <svg class="h-6 w-6 text-red-500 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        <p class="text-red-700 font-medium" id="error-text-{{ $jobOffer->id }}">An error occurred. Please try again.</p>
                    </div>
                </div>
            </form>
        </div>
