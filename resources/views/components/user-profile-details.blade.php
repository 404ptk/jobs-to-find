...
<div class="px-8 pb-8 -mt-16">
    <div class="flex items-center gap-6 mb-6">
        <div class="w-32 h-32 rounded-full overflow-hidden border-4 border-white shadow-lg bg-white flex-shrink-0 relative z-10">
        ...
        <div class="flex-1 mt-16">
            <div class="flex items-center gap-3 mb-2">
                ...
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold {{ $user->account_type === 'job_seeker' ? 'bg-blue-100 text-blue-700' : ($user->account_type === 'employer' ? 'bg-emerald-100 text-emerald-700' : 'bg-purple-100 text-purple-700') }}">
    ...
    @if($user->account_type === 'job_seeker' && $user->skills->count() > 0)
    <div class="mb-6">
        ...
        <div>
            ...
            <div class="space-y-2 bg-gray-50 p-4 rounded-lg">
                <div class="flex items-center text-sm">
                    ...
                    @if($user->github_url)
                        <a href="{{ $user->github_url }}" target="_blank" class="text-blue-600 hover:underline truncate">{{ Str::limit($user->github_url, 30) }}</a>
                    @else
                ...
                <div class="flex items-center text-sm">
                    ...
                    @if($user->linkedin_url)
                        <a href="{{ $user->linkedin_url }}" target="_blank" class="text-blue-600 hover:underline truncate">{{ Str::limit($user->linkedin_url, 30) }}</a>
                    @else
                        <span class="text-gray-400">Not connected</span>
                    @endif
                </div>
    ...
    <div class="border-t border-gray-200 pt-4 text-sm text-gray-500">
        ...
        <a href="{{ route('user.show', $user->username) }}" class="block w-full px-4 py-2.5 bg-blue-600 text-white text-center font-medium rounded-lg hover:bg-blue-700 transition">
            Go to profile page
        </a>
    </div>
</div>
