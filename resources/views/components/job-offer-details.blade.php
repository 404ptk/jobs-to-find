                ...
                <div class="flex items-start">
                    ...
                    <div>
                        ...
                        <p class="text-gray-700">
                            ...
                            @else
                                This job offer is currently under review by our administrators. It will be publicly visible once
                                approved.
                            @endif
                        </p>
                    </div>
                    ...
                    <section>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($jobOffer->skills as $skill)
                                <span
                                    class="px-4 py-2 bg-blue-50 text-blue-700 rounded-full text-sm font-semibold border border-blue-100 shadow-sm hover:bg-blue-100 transition duration-200">
                                    {{ $skill->name }}
                                </span>
                            @endforeach
                        </div>
                ...
                <div class="lg:hidden mt-8">
                    <div class="bg-gray-50 rounded-lg p-6 mb-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <p class="font-medium text-gray-900">{{ $jobOffer->is_remote ? 'Remote' : 'On-site' }}
                                </p>
                            </div>
                            <div class="pt-4 border-t border-gray-200">
                                <div class="flex items-center justify-between text-sm mb-2">
                                    ...
                                    <span
                                        class="font-semibold text-gray-900">{{ number_format($jobOffer->views_count) }}</span>
                                </div>
                                <div class="flex items-center justify-between text-sm">
                                    ...
                                    <span
                                        class="font-semibold text-gray-900">{{ number_format($jobOffer->applications->count()) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 rounded-lg p-6 mb-6">
                        ...
                        <div class="flex items-center gap-3 mb-3">
                            @if($jobOffer->company && $jobOffer->company->logo_path)
                                <img src="{{ asset('storage/' . $jobOffer->company->logo_path) }}"
                                    alt="{{ $jobOffer->company->name }}" class="w-10 h-10 rounded-lg object-cover border border-gray-100">
                            @else
                ...
                @auth
                    @if(Auth::user()->account_type === 'admin' && !$jobOffer->is_approved)
                        <div class="mt-8 bg-yellow-50 border-2 border-yellow-400 rounded-lg p-6">
                            ...
                            <p class="text-gray-700 mb-4">This job offer is awaiting approval. Review and approve or reject it.
                            </p>
                            <div class="flex gap-3 mb-4">
                                <form action="{{ route('admin.approve-offer', $jobOffer->id) }}" method="POST" class="flex-1">
                                    @csrf
                                    <button type="submit"
                                        class="w-full px-6 py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition flex items-center justify-center cursor-pointer">
                                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <form action="{{ route('admin.reject-offer', $jobOffer->id) }}" method="POST" class="flex-1"
                                    id="reject-form-{{ $jobOffer->id }}">
                                    @csrf
                                    <button type="button" data-action="reject" data-offer-id="{{ $jobOffer->id }}"
                            ...
                            <button onclick="openMessageModal({{ $jobOffer->user_id }})"
                                class="w-full px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition flex items-center justify-center cursor-pointer">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @elseif(Auth::user()->account_type === 'job_seeker')
                        ...
                        @if($existingApplication)
                            <div class="mt-8 bg-green-50 border border-green-200 rounded-lg p-6">
                                ...
                                <p class="text-gray-600 text-center mt-2">Applied on
                        ...
                        @else
                            <div class="mt-8 bg-blue-50 border border-blue-200 rounded-lg p-6">
                                ...
                                <p class="text-gray-700 mb-4">Submit your application for this position and our team will get back
                                    to you soon.</p>
                                <button onclick="showApplyModal({{ $jobOffer->id }})"
                                    class="w-full px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition flex items-center justify-center cursor-pointer">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    ...
                    @elseif(Auth::user()->account_type === 'admin')
                        <div class="bg-red-50 border border-red-200 rounded-lg p-6">
                            ...
                            <button onclick="openMessageModal({{ $jobOffer->user_id }})"
                                class="w-full px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition flex items-center justify-center cursor-pointer mb-3">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                                </svg>
