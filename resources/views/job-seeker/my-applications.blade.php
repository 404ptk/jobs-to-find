...
@section('content')
    <div class="min-h-[calc(100vh-8rem)] py-12 px-4">
        <div class="max-w-7xl mx-auto">
            ...
            <x-toolbar :total="$applications->total()" routeName="my-applications" gridId="applicationsGrid"
                :defaultColumns="3" />
                                    ...
                                    <div class="p-6">
                                        <div class="flex items-start justify-between mb-4">
                                            <div class="flex-1">
                                                ...
                                                @endphp
                         <span
                                                    class="inline-block px-2 py-1 text-xs font-medium rounded-full {{ $statusColor }} mb-2">
                                        ...
                                        <div class="space-y-2 mb-4">
                                            <div class="flex items-center text-sm text-gray-600">
                                                ...
                                                {{ $application->jobOffer->location->city }},
                                                {{ $application->jobOffer->location->country }}
                                            </div>
                                            ...
                                            @if($application->jobOffer->salary_min || $application->jobOffer->salary_max)
                                                <div class="flex items-center text-sm text-gray-600">
                                                    ...
                                                    @if($application->jobOffer->salary_min && $application->jobOffer->salary_max)
                                                        {{ number_format($application->jobOffer->salary_min, 0) }} -
                                                        {{ number_format($application->jobOffer->salary_max, 0) }}
                                                        {{ $application->jobOffer->currency }}
                                                    @elseif($application->jobOffer->salary_min)
                                                        From {{ number_format($application->jobOffer->salary_min, 0) }}
                                                        {{ $application->jobOffer->currency }}
                                                    @else
                                                        Up to {{ number_format($application->jobOffer->salary_max, 0) }}
                                                        {{ $application->jobOffer->currency }}
                                                    @endif
                                                </div>
                                            @endif
                                        ...
                                        <div class="pt-4 border-t border-gray-100">
                                            ...
                                            <button onclick="openJobModal({{ $application->jobOffer->id }})"
                                                class="block w-full px-4 py-2 bg-gray-100 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-200 transition text-center cursor-pointer">
                                                View Job Details
                                            </button>
                                        </div>
                                    </div>
    ...
    @foreach($applications as $application)
        <div id="job-modal-{{ $application->jobOffer->id }}"
            class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
            <div class="fixed inset-0 bg-opacity-30 backdrop-blur-sm transition-opacity"
                onclick="hideJobModal({{ $application->jobOffer->id }})"></div>
            <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto transform transition-all scale-95 opacity-0 relative"
                id="job-modal-content-{{ $application->jobOffer->id }}">
                <button onclick="hideJobModal({{ $application->jobOffer->id }})"
