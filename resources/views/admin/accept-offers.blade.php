...
@section('content')
    <div class="min-h-[calc(100vh-8rem)] py-12 px-4">
        <div class="max-w-7xl mx-auto">
            ...
            <x-toolbar :total="$pendingOffers->total()" routeName="admin.accept-offers" gridId="acceptOffersGrid"
                :defaultColumns="3" />
                        ...
                        <div class="bg-white rounded-lg shadow-sm hover:shadow-md transition border-2 border-yellow-200">
                            <div class="p-6">
                                <div class="flex items-start justify-between mb-4">
                                    <div class="flex-1">
                                        <span
                                            class="inline-block px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800 mb-2">
                                            Pending Approval
                                        </span>
                                ...
                                <div
                                    class="flex items-center justify-between text-xs text-gray-500 mb-4 pt-4 border-t border-gray-100">
                                    <span>Submitted {{ $offer->created_at->diffForHumans() }}</span>
                                </div>

                                <div class="flex gap-2">
                                    <button onclick="openOfferModal({{ $offer->id }})"
                                        class="flex-1 px-4 py-2 bg-gray-100 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-200 transition text-center cursor-pointer">
                                        View Details
                                    </button>
                                </div>

                                <div class="flex gap-2 mt-3">
                                    <button onclick="openModal('confirm-modal', 'approve-form-{{ $offer->id }}')"
                                        class="flex-1 px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition cursor-pointer">
                                        Approve
                                    </button>
                                    <form id="approve-form-{{ $offer->id }}" action="{{ route('admin.approve-offer', $offer->id) }}"
                                        method="POST" class="hidden">
                                        @csrf
                                    </form>

                                    <button onclick="openModal('delete-modal', 'reject-form-{{ $offer->id }}')"
                                        class="flex-1 px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 transition cursor-pointer">
                                        Reject
                                    </button>
                                    <form id="reject-form-{{ $offer->id }}" action="{{ route('admin.reject-offer', $offer->id) }}"
                                        method="POST" class="hidden">
                                        @csrf
                                    </form>
                                </div>
    ...
    <x-modal-delete name="delete-modal" title="Reject Offer"
        message="Are you sure you want to reject this offer? This action cannot be undone." confirmText="Reject"
        cancelText="Cancel" />

    <x-modal-confirm name="confirm-modal" title="Approve Offer"
    ...
    <div id="offer-details-modal"
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto transform transition-all scale-95 opacity-0 pointer-events-auto relative"
            id="offer-modal-content">
            <button onclick="hideOfferModal()"
                class="absolute top-4 right-4 text-white hover:text-gray-200 z-10 cursor-pointer">
