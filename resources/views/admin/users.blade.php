    ...
    <div class="min-h-[calc(100vh-8rem)] py-12 px-4">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <x-stat-card title="Employers" :value="$totalEmployers" color="green">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <x-stat-card title="Job Seekers" :value="$totalJobSeekers" color="purple">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
                <x-toolbar :total="$users->total()" routeName="admin.users" gridId="usersTable" :showGridButtons="false" />
            ...
            <x-table :headers="[
            ['label' => 'User', 'sort' => 'first_name'],
            ['label' => 'Role', 'sort' => 'account_type'],
            ['label' => 'Country', 'sort' => 'country'],
            ['label' => 'Joined', 'sort' => 'created_at']
        ]" :pagination="$users" :alignLastRight="false" :sort="$sort" :direction="$direction">
                @forelse($users as $user)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="shrink-0 h-10 w-10 rounded-full overflow-hidden border border-gray-300">
                                    @if($user->avatar)
    ...
    <script>
        ...
        function openUserModal(userId) {
            ...
            fetch(`/admin/user/${userId}/partial`)
                ...
                .catch(error => {
                    userModalBody.innerHTML = '<p class="text-red-500 text-center">Error loading profile. Please try again.</p>';
