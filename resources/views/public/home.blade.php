...
@section('content')
    <div id="mouse-bg" class="min-h-[calc(100vh-8rem)] flex items-center justify-center px-4 relative overflow-hidden">
        <div id="radial-gradient" class="absolute inset-0 pointer-events-none transition-opacity duration-300"
            style="background: radial-gradient(600px circle at 50% 50%, rgba(0, 0, 0, 0.15), transparent 80%);"></div>
                ...
                <form action="/search" method="GET" class="space-y-4">
                    <div class="flex flex-col md:flex-row gap-4">
                        <div class="flex-1">
                            ...
                            <input type="text" id="search" name="search"
                                placeholder="e.g. Frontend Developer, Marketing Manager..." pattern="[a-zA-Z0-9\s\-]+"
                        ...
                        <div class="flex-1">
                            ...
                            <select id="employment_type" name="employment_type"
                                ...
                                @foreach($employmentTypes as $type)
                                    <option value="{{ $type }}">
                                        {{ $type === 'b2b' ? 'B2B' : ucfirst(str_replace('-', ' ', $type)) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="pt-2">
                        <button type="submit"
                            ...
                            <span
                                class="absolute inset-0 bg-blue-700 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 ease-out origin-center blur-md"></span>
