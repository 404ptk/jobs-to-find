...
@section('content')
    <div class="min-h-[calc(100vh-8rem)] py-12 px-4">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                ...
                <form method="GET" action="{{ route('search') }}" id="search-form">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <select name="sort"
                                ...
                                <option value="salary_high" {{ request('sort') == 'salary_high' ? 'selected' : '' }}>Highest
                                    salary</option>
                                <option value="salary_low" {{ request('sort') == 'salary_low' ? 'selected' : '' }}>Lowest
                                    salary</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3 mt-4">
                        <button type="submit"
                            class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-medium cursor-pointer">
                            Apply Filters
                        </button>
                        <a href="{{ route('search') }}"
            ...
            @else
                <x-toolbar :total="$jobOffers->total()" :currentPerPage="10" :perPageOptions="[10, 20, 30]" routeName="search"
                    gridId="jobOffersGrid" storageKey="jobOffersGridLayout" :defaultColumns="2" />

                <div id="jobOffersGrid" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    @foreach($jobOffers as $offer)
                        <div
                            class="bg-white rounded-lg shadow-sm hover:shadow-md transition border border-gray-200 hover:bg-gray-100 flex flex-col">
                            <div class="p-6 flex flex-col flex-1">
                                <div class="flex items-start justify-between mb-4">
                                    <div class="flex-1">
                                        <a href="{{ route('job.show', $offer->id) }}">
                                            <h3
                                                class="text-xl font-semibold text-gray-900 mb-2 hover:text-blue-600 transition cursor-pointer">
                                                {{ $offer->title }}
                                            </h3>
                                        </a>
                                ...
                                <div
                                    class="flex items-center justify-between text-xs text-gray-500 mb-4 pt-4 border-t border-gray-100 mt-auto">
                                ...
                                <div class="flex gap-3">
                                    @guest
                                        <a href="/login"
                                            class="px-3 py-2 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition"
                                    ...
                                    @auth
                                        @if(Auth::user()->account_type === 'job_seeker')
                                            ...
                                            @else
                                                <button onclick="showApplyModal({{ $offer->id }})"
                                                    class="px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition cursor-pointer">
                                                    Apply Now
                                                </button>
                                            @endif
                                        @endif
