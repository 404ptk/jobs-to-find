            ...
            <div class="bg-white rounded-lg shadow-sm p-8">
                <form action="{{ route('offer.update', $jobOffer->id) }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title', $jobOffer->title) }}"
                        ...
                        <div>
                            ...
                            <select id="parent_category" name="parent_category"
                                ...
                                @foreach($parentCategories as $parent)
                                    <option value="{{ $parent->id }}" {{ old('parent_category', $jobOffer->category->parent_id) == $parent->id ? 'selected' : '' }}>
                                        {{ $parent->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select id="category_id" name="category_id"
                                ...
                                @foreach($subCategories as $sub)
                                    <option value="{{ $sub->id }}" 
                                        ...
                                        {{ old('category_id', $jobOffer->category_id) == $sub->id ? 'selected' : '' }}>
                                        {{ $sub->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <input type="number" id="salary_min" name="salary_min"
                                    value="{{ old('salary_min', $jobOffer->salary_min) }}" step="10" min="0"
                            ...
                            <div>
                                <input type="number" id="salary_max" name="salary_max"
                                    value="{{ old('salary_max', $jobOffer->salary_max) }}" step="10" min="0"
                    ...
                    <div class="mb-8">
                        ...
                        <textarea id="requirements" name="requirements" rows="8"
                            ...
                            required>{{ old('requirements', $jobOffer->requirements) }}</textarea>
                        @error('requirements')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-sm text-gray-500">Press Enter for line breaks</p>
                        ...
                        <div class="flex items-start">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-blue-700 mt-1">
                                    This offer was originally posted on {{ $jobOffer->created_at->format('F d, Y') }}.
                                    Editing will not change the post date.
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            function filterPositions() {
                ...
                positionOptions.forEach(option => {
                    ...
                    if (option.dataset.parent === selectedParentId) {
                        const newOption = option.cloneNode(true);
                        if (newOption.value === currentPositionId.toString()) {
                            newOption.selected = true;
